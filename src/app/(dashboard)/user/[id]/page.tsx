...
import { addMonths, addYears, getNextPaymentDate } from "@/app/utils/utils";
...
export default function UserPage() {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      showAlert("error", `Error al cancelar suscripción: ${error.message}`);
    } finally {
      setCancelling(false);
    }
  };
  ...
  const canCancelNextHypnosis = () => {
    ...
    // Solo permitir si tiene una membresía activa y una fecha de próxima hipnosis
    const hasActiveMembership = getMembershipStatus(user.lastMembership) === "Activa";
    const hasNextHypnosisDate = user.features?.nextAvailableForm?.value;
    
    return hasActiveMembership && hasNextHypnosisDate;
  };
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <CopyText
                    text={`${formatDateUTC(
                      user?.lastMembership 
                        ? getNextPaymentDate(user.lastMembership)
                        : "Sin fecha"
                    )}`}
                    className="text-[14px] font-bold flex items-center gap-[5px]"
                    iconClassName="hidden"
